...
/* Se bloat_pct > 30%, procedo con pg_repack invece di VACUUM FULL */ /* Installo pg_repack come estensione */
CREATE EXTENSION pg_repack;

/* Eseguo la ricostruzione online: nessun lock esclusivo sulla tabella */ pg_repack - h localhost - d mydb - t bloated_table /* Verifico la riduzione di spazio dopo il repack */
SELECT
  pg_size_pretty(pg_total_relation_size('bloated_table')) AS size_after;

/* Ricostruisco anche gli indici della tabella in un unico passaggio */ pg_repack - h localhost - d mydb - t bloated_table --only-indexes
```
