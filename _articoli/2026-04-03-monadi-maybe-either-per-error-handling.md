...
// Applica fn se sono Right chain: fn => fn(value), 
// Come map ma fn restituisce già un Either getOrElse: _ => value, 
// Estrae il valore ignorando il default fold: (leftFn, rightFn) =>
// rightFn(value),
...
// Estrae il default poiché sono in errore fold: (leftFn, _) => leftFn(error), 
// Applica la funzione di sinistra toString: () => `Left($
{error}
)` }
); 
...
; 
* Pipeline di validazione che usa Either per gestire gli errori senza try/catch.
* */
...
.chain(name => validateEmail(input.email) .chain(email => validateAge(input.age)
.map(age => (
{ name, email, age }
)) 
// Costruisco l'oggetto validato ))); }
 
* Uso la pipeline nella gestione delle richieste Express senza nessun try/catch.
* */
...
* Per operazioni asincrone, implemento AsyncEither (o uso una libreria come fp-
* ts). */
...
{ const exists = await db.users.findByEmail(validData.email); return exists ?
Left('Email già registrata') : Right(validData); }
) .chain(async data => 
{ const user = await db.users.create(data); await
emailService.sendWelcome(user.email); return Right(user); }
); }
```
