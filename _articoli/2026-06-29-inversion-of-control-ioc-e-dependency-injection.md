...
class PostgresUserRepository extends UserRepository {
    ...
    async findById(id) {
        const row = await this.db.query('SELECT * FROM users WHERE id = $1',
            [id]);
        return row.rows[0] ? new User(row.rows[0]) : null;
    }
    async save(user) {
        await this.db.query('INSERT INTO users(id, name, email)
            VALUES($1,$2,$3)', [user.id, user.name, user.email]);
    }
}
...
class UserService {
    constructor(userRepo, emailService) {
        // Inietto le dipendenze: il servizio è completamente ignaro
            dell'implementazione this.userRepo = userRepo
        ;
...
describe('UserService', () => {
    it('dovrebbe registrare un nuovo utente', async () => {
        ...
        expect(emailSpy.sendWelcome).toHaveBeenCalledWith('alice
        @example.com');
    }
    );
}
