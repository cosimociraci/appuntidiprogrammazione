...
```db
/* Dimostro l'impatto della volatilità con un esempio concreto. Questa funzione è VOLATILE per default: il Planner la chiama una volta per riga, anche se l'input è costante. */
CREATE OR REPLACE FUNCTION convert_currency (amount numeric, from_cur text, to_cur text) RETURNS numeric AS $$ SELECT amount * rate FROM exchange_rates WHERE from_currency = from_cur AND to_currency = to_cur; $$ LANGUAGE SQL VOLATILE;
