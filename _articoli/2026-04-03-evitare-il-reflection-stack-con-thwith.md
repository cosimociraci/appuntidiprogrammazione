...
sintesi: "Accedere ripetutamente a metodi complessi tramite SpEL all'interno di un ciclo  costoso. L'istruzione th-with crea una variabile locale nel contesto di Thymeleaf. Memorizzando il risultato di una computazione o di una lookup una sola volta per itera"
tech: "thymeleaf"
...
"
    th:with=" subtotal=$
{order.items.stream().mapToDouble(i -> i.price * i.quantity).sum()}
, hasDiscount=$
{order.coupon != null and order.coupon.active}
, discountAmount=$
{order.coupon != null}
 ? $
{subtotal * order.coupon.rate}
 : 0, finalTotal=$
...
"
        ...
        <td
            th:text="$
{#finance.formatCurrency(subtotal, 'EUR')}
"
        >
            Subtotale
        </td>
