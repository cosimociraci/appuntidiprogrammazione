    ...
    fallimento public void safeMethod() throws InterruptedException
{
    ...
    if (!acquired) {
        ...
        throw new ServiceUnavailableException("Risorsa occupata, riprova più
            tardi");
    }
    try {
        externalService.call();
...
@Service public class SharedResourceService {
    ...
    public <T> T executeWithLock(Supplier<T> operation) {
        ...
        try {
            acquired = resourceLock.tryLock(LOCK_TIMEOUT_MS,
                TimeUnit.MILLISECONDS);
            if (!acquired) {
                // Monitoro il numero di timeout per rilevare saturazione della
                    risorsa meterRegistry.counter("lock.timeout", "resource",
                    "shared").increment()
                ;
                throw new ResourceBusyException("Risorsa occupata dopo " +
                    LOCK_TIMEOUT_MS + "ms");
            }
            ...
            throw new ServiceUnavailableException("Thread interrotto durante
                l'attesa del lock");
        }
        finally {
            if (acquired) {
...
if (deadlockedThreads != null) {
    ...
    for (ThreadInfo info : infos) {
        ...
        log.error("Lock che il thread aspetta: {
        }
        ", info.getLockName());
        log.error("Lock owner: {
        }
...
@Scheduled(fixedDelay = 60_000) public void detectDeadlocks() {
    ...
    if (deadlocked != null) {
        alertService.sendCritical("DEADLOCK rilevato su " + deadlocked.length +
            " thread. Riavvio necessario.");
    }
}
/* Strategia di lock ordering per prevenire i deadlock strutturalmente:
    acquisire sempre i lock nello stesso ordine globale. */
public void transferFunds(Account from, Account to, BigDecimal amount) {
    // Ordino i lock per ID per garantire che due thread che trasferiscono tra
        gli stessi account // acquisiscano sempre i lock nello stesso ordine,
        prevenendo il deadlock. Account first = from.getId() < to.getId() ? from
        : to
    ;
    ...
    try {
        firstAcquired = first.getLock().tryLock(1, TimeUnit.SECONDS);
        if (!firstAcquired) throw new ResourceBusyException("Account " +
            first.getId() + " occupato");
        secondAcquired = second.getLock().tryLock(1, TimeUnit.SECONDS);
        if (!secondAcquired) throw new ResourceBusyException("Account " +
            second.getId() + " occupato");
        from.debit(amount);
        to.credit(amount);
    }
    catch (InterruptedException e) {
        ...
        if (secondAcquired) second.getLock().unlock();
        if (firstAcquired) first.getLock().unlock();
    }
}
```
