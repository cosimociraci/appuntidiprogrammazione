...
## Analisi Tecnica
**Problema:** Blocco dell'Event Loop dovuto a task sincroni CPU-bound che impediscono il rendering o la gestione di I/O.

**Perché:** Delegazione asincrona via Workers. Ho scelto di spostare il calcolo pesante su un thread isolato, usando i messaggi solo per i risultati finali, evitando la contesa sulla memoria principale.
...
async function parseLargeCsv(filePath) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // Trasferisco l'ownership: fileBuffer non è più accessibile nel main
            thread worker.postMessage(
        {
            buffer: fileBuffer.buffer, filePath
        }
        , [fileBuffer.buffer]
        // Lista dei Transferable: trasferiti, non copiati )
        ;
...
// Ricevo il buffer trasferito: ho l'ownership completa, zero contesa con il
    main thread const buffer = Buffer.from(workerData.buffer)
;
...
function parseCsv(text) {
    ...
    i++) {
        ...
        rows.push(row);
        // Invio aggiornamenti di progresso ogni 10000 righe senza aspettare la
            fine if (i % 10_000 === 0)
        {
...
;
const resultBuffer = Buffer.from(resultJson).buffer;
parentPort.postMessage( {
    ...
    return new Promise((resolve, reject) => {
        ...
        worker.onmessage = (e) => {
            if (e.data.type === 'progress') {
                ...
                const result =
                    JSON.parse(Buffer.from(e.data.buffer).toString());
                resolve(result);
                worker.terminate();
            }
        }
