...
@SupportedSourceVersion(SourceVersion.RELEASE_17) public class MapperProcessor
        ...
        for (Element element :
            ...
            // Genero il file sorgente del mapper try
            {
                ...
                processingEnv.getMessager().printMessage( Diagnostic.Kind.ERROR,
                    "Errore generazione mapper: " + e.getMessage(), element);
            }
        }
        return true;
    }
    private void generateMapperClass(TypeElement sourceClass, GenerateMapper
        ...
        String mapperName = sourceClass.getSimpleName() + "Mapper";
        ...
        try (PrintWriter writer = new PrintWriter(file.openWriter())) {
            ...
            ;
            writer.println("public class " + mapperName + " {
                ");
                // Genero i metodi di mapping per ogni campo della classe
                    sorgente for (Element field :
                    sourceClass.getEnclosedElements())
                {
                    if (field.getKind() == ElementKind.FIELD) {
                        ...
                        writer.println(" public void map" + capitalizedField +
                            "(Object source, Object target) {
                            ");
                            writer.println("
                            // Mapping generato a compile time: zero
                                reflection")
                            ;
                            writer.println("
                        }
...
/* Registro il processor nel file
...
@Service public class OrderService {
    // Inietto il mapper generato: è codice Java puro, zero reflection private
        final OrderMapper mapper = new OrderMapper()
    ;
    public OrderDto toDto(Order order) {
        OrderDto dto = new OrderDto();
        mapper.mapId(order, dto);
        mapper.mapStatus(order, dto);
        mapper.mapTotal(order, dto);
        return dto;
