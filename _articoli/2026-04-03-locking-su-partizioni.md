...
AS partition_bound FROM pg_inherits JOIN pg_class c ON inhrelid = c.oid WHERE
inhparent = 'log_data'::regclass ORDER BY partition_bound;
* Distacco la partizione vecchia senza bloccare i nuovi inserimenti.
* CONCURRENTLY è un'operazione in due fasi: prima marca la partizione come
* "detaching", poi completa il distacco quando tutte le transazioni attive sono
* concluse. */
...
* partizione è visibile in pg_partitioned_table con stato 'd'. */
 SELECT relname, pg_get_expr(relpartbound, oid) FROM pg_class WHERE relname =
'log_data_2023_01';
* Una volta distaccata, la partizione diventa una tabella indipendente. Posso
* archiviarla o eliminarla in modo completamente asincrono rispetto al traffico
