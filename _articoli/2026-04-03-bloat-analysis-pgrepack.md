...
n_dead_tup, 0) * 100, 2) AS bloat_pct FROM pg_stat_user_tables WHERE n_dead_tup
> 10000 ORDER BY bloat_pct DESC;
/* Se bloat_pct > 30%, procedo con pg_repack invece di VACUUM FULL */
 
/* Installo pg_repack come estensione */
 CREATE EXTENSION pg_repack; 
/* Eseguo la ricostruzione online: nessun lock esclusivo sulla tabella */
 pg_repack -h localhost -d mydb -t bloated_table 
/* Verifico la riduzione di spazio dopo il repack */
 SELECT pg_size_pretty(pg_total_relation_size('bloated_table')) AS size_after; 
