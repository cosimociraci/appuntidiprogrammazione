...
* grandi */
 ALTER SYSTEM SET maintenance_work_mem = '512MB'; SELECT pg_reload_conf(); 
* Verifico che i nuovi INSERT riutilizzino le pagine libere invece di allocarne
* di nuove */
 SELECT relpages, pg_size_pretty(pg_relation_size('my_table')) AS table_size
FROM pg_class WHERE relname = 'my_table';
* Confronto relpages prima e dopo un ciclo delete+insert+vacuum: deve restare
* stabile */
```
